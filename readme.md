...
#### <span dir="ltr"> dispatch_random(self) </span>

این تابع بین صف های موجود که خالی نیستند، یکی را به صورت تصادفی برمیگرداند. از این تابع در بخش امتیازی استفاده می شود.
...
البته در حالتی که ورودی BONUS را ۱ داده باشیم، بخش های امتیازی هم در این متد انجام می شود.
به این صورت که در هنگام انتخاب صف، بین صف های پر یکی به صورت تصادفی انتخاب می شود و پردازه بعدی از آن انتخاب می شود.

همچنین برای پیاده سازی timeout، زمان تشکیل پردازه را در آن ذخیره می کنیم و اگر که از آن عبور کرده باشیم،
پردازه دیگر انجام نمی شود و از صف پردازه ها حذف می شود.
البته یک کلاک برای این عمل مصرف می شود.
...
#### <span dir="ltr"> def note_timeout(self, time: int) </span>

هرگاه که یک پردازه timeout شود، این متد زمان رخداد آن را ذخیره می کند.

#### <span dir="ltr"> calculate_timeouts(self) </span>

در انتهای شبیه سازی، این متد نمودار زمانی رخداد timeout ها را رسم می کند.
...
برنامه در کنسول از شما یک سری ورودی می خواهد که به شرح زیر است:

<ul>

<li>Total Time : کل زمانی است که میخواهید شبیه سازی اجرا شود.</li>
<li>Y: میانگین توزیع نمایی زمان سرویس تسک ها است.</li>
<li>X: نرخ تولید تسک ها در فرایند پواسون است.</li>
<li>K: هر گاه کمتر از این تعداد پردازه در صف ها داشته باشیم، پردازه های جدید وارد می شوند.</li>
<li>T1: کوانتوم تایم صف RR1 است.</li>
<li>T2: کوانتوم تایم صف RR2 است.</li>
<li>BONUS mode: اگر ۱ باشد، بخش های امتیازی پروژه فعال می شوند.</li>
<li>Z: مدت timeout برای بخش امتیازی است.</li>
</ul>

یک مثال از ورودی دادن می تواند به صورت زیر باشد:
...
average time spent in RR1 is 60.06
average time spent in RR2 is 34.75
average time spent in FCFS is 112.6
a total of 13 jobs timed out...
```

</div>

پیشنهادمان هم برای بهبود مدت زمان انتظار در RR ها، بیشتر بودن زمان RR1 نسبت به RR2 است.
چون پردازه هایی که وارد RR2 می شوند به طور ناگهانی وقفه طولانی در اجرایشان پیش می آید و بهتر است که پردازه های کمتری وارد این صف شوند.
این فرضیه با دادن ورودی هایی با این شرط به شبیه ساز هم در خروجی مشاهده می شود.
